...
`Использовать ИИ для заполнения статусов и описаний? [y/N]`. Только `y`
отправляет историю каждого тикета в ИИ и записывает краткий содержательный
результат (не более 600 символов) в «Текущий статус/решение». Если заголовок не
соответствует шаблону `Запрос обновления <версия>`, заголовок вместе с исходным
описанием карточки отправляется отдельным запросом, а одно краткое предложение
записывается в «Описание». При трёх неудачных попытках отдельного AI-запроса
выгрузка продолжается, а соответствующая ячейка остаётся пустой.
